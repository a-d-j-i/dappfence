...
**Synthetic `#scripts` keys** — a key of the form `pageKey + "#scripts"` holds the valid inline
script hashes for that page. Browsers always strip URL fragments before SW dispatch, so `#` can
never appear in a real request key — making it an unambiguous namespace for inline script entries.
The value is an array of valid content hashes; each extracted inline script must match one entry
(set-membership, not positional). See [Inline script verification](#inline-script-verification).
